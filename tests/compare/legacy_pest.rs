...
use cmakefmt::parser::ast::{
    Argument, BracketArgument, CommandInvocation, Comment, File, Statement,
};
...
mod generated {
    ...
    #[derive(Parser)]
    #[grammar = "tests/compare/cmake_legacy.pest"]
...
fn parse_internal(source: &str) -> std::result::Result<File, LegacyDiagnostic> {
    let mut pairs = CmakeParser::parse(Rule::file, source).map_err(|e| from_pest(&e))?;
    let file_pair = pairs.next().ok_or_else(|| LegacyDiagnostic {
        message: "parser did not return a file pair".into(),
        line: 1,
        column: 1,
    })?;
...
) -> std::result::Result<File, LegacyDiagnostic> {
...
fn collect_file_item(
    ...
    match item.as_rule() {
        ...
        other => Err(LegacyDiagnostic {
            message: format!("unexpected top-level parser rule: {other:?}").into_boxed_str(),
            line: 1,
            column: 1,
        }),
    }
...
fn build_command(
    ...
    for inner in pair.into_inner() {
        match inner.as_rule() {
            ...
            Rule::space => {}
            other => {
                return Err(LegacyDiagnostic {
                    message: format!("unexpected command parser rule: {other:?}").into_boxed_str(),
                    line: 1,
                    column: 1,
                });
            }
    ...
    Ok(CommandInvocation {
        name: name.ok_or_else(|| LegacyDiagnostic {
            message: "command missing identifier".into(),
            line: 1,
            column: 1,
        })?,
        arguments,
...
fn collect_argument_part(
    ...
    match pair.as_rule() {
        ...
        Rule::argument => {
            ...
            let argument = inner.next().ok_or_else(|| LegacyDiagnostic {
                message: "argument missing child node".into(),
                line: 1,
                column: 1,
            })?;
        ...
        Rule::space => Ok(()),
        other => Err(LegacyDiagnostic {
            message: format!("unexpected argument parser rule: {other:?}").into_boxed_str(),
            line: 1,
            column: 1,
        }),
    }
...
fn build_argument(
    ...
    match pair.as_rule() {
        ...
        other => Err(LegacyDiagnostic {
            message: format!("unexpected argument rule: {other:?}").into_boxed_str(),
            line: 1,
            column: 1,
        }),
    }
...
fn validate_bracket_argument(
    raw: String,
) -> std::result::Result<BracketArgument, LegacyDiagnostic> {
    let open_equals = raw
        .strip_prefix('[')
        .ok_or_else(|| LegacyDiagnostic {
            message: "bracket argument missing '[' prefix".into(),
            line: 1,
            column: 1,
        })?
        .bytes()
    ...
    let close_equals = raw
        .strip_suffix(']')
        .ok_or_else(|| LegacyDiagnostic {
            message: "bracket argument missing ']' suffix".into(),
            line: 1,
            column: 1,
        })?
        .bytes()
    ...
    if open_equals != close_equals {
        return Err(LegacyDiagnostic {
            message: format!("invalid bracket argument delimiter: {raw}").into_boxed_str(),
            line: 1,
            column: 1,
        });
    }
