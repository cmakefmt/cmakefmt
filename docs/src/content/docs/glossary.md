...
- **no_autosort** — A property a command spec attaches to a keyword section to exempt that section from the [autosort](#formatter-concepts) heuristic. Used for sections whose value list has positional semantics that flat sorting would silently corrupt — e.g. `PROPERTY <name> <values…>` in `set_property` or the `<key> <value>` pair structure under `PROPERTIES`. An explicit `sortable: true` on the same section still wins; `no_autosort` only protects against the heuristic.
